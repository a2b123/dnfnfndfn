...
class AFCWildcardCell: BaseCell {
    
    var afcWildcard: TeamModel? {
        didSet {
            
            if let teamImageView = afcWildcard?.teamLogo {
                teamLogo.image = teamImageView
            }
            
            teamLabel.text = afcWildcard?.teamName
            teamRecordLabel.text = afcWildcard?.teamRecord
            if let seed = afcWildcard?.seed {
                seedLabel.text = "\(seed)"
            }
...
//        label.backgroundColor = .yellow
    ...
    override func setupView() {
        ...
        _ = seedLabel.anchor(nil, left: nil, bottom: nil, right: rightAnchor, topConstant: 0, leftConstant: 0, bottomConstant: 0, rightConstant: 12, widthConstant: 50, heightConstant: 50)
